...
import { unzip } from "https://svgmap.github.io/svgmapAppLayers/commonLib/unzipit.module.js";
...
async function initializeLayer() {
  ...
  await loadArchive(getArchiveById("latest"));
}
...
function bindUi() {
  ui.load.addEventListener("click", () => loadArchive(getArchiveById(ui.archive.value)));
  ui.refresh.addEventListener("click", () => {
    ui.archive.value = "latest";
    void loadArchive(getArchiveById("latest"));
  });
...
async function loadArchive(archive) {
  ...
  try {
    const response = await fetch(svgMap.getCORSURL(archive.url), {
